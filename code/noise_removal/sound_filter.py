...
def reduce_noise_power(y, sr):
    ...
    # less_noise = AudioEffectsChain().lowshelf(gain=-12.0, frequency=threshold_l, slope=0.5).highshelf(gain=-12.0, frequency=threshold_h, slope=0.5).limiter(gain=10.0)
...
filename = '../datasets/songs/ride_mp3.wav'
y, sr = librosa.load(filename)
y_reduced_centroid_mb = reduce_noise_centroid_mb(y, sr)
y_reduced_power = enhance(y_reduced_centroid_mb)
y_reduced_power, time_trimmed = trim_silence(y_reduced_power)
librosa.output.write_wav('../datasets/songs/ride_enhance_cmb.wav', y, sr)